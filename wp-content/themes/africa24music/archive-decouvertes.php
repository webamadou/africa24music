...
<div class="container">
    <div class="cadre"> <!-- cadre contenu des pages: mettre le contenu des pages à partir d'ici -->
        <div class="page-content">
            <h1 class="page-title">dernieres vidéos</h1>
                    ...
                    <ul id="pays-list" class="liste pays">
                        <?php 
                            ...
                            foreach ($pays as $tag) {
                                ...
                                ?>
                                <li class="col-xs-6 col-sm-4 col-md-3 pays" data-group = "<?php echo 'categ'.$c ?>"><a href="<?php echo $term_link ; ?>" class="tax-filter" data-term="<?php echo $tag->slug; ?>" data-tax="pays" ><?php echo ucfirst($tag->name); ?></a></li>
                        <?php  
                            $c++ ;
                            $c = ($c <= 5)? $c : 1 ;
                        }
                    ...
                    <ul id="genres-list" class="liste genres">
                        <?php 
                            ...
                            foreach ($genres as $tag) {
                                ...
                                <li class="col-xs-6 col-sm-4 col-md-3 genre" data-group = "<?php echo 'categ'.$c ?>"><a href="<?php echo $term_link ; ?>" data-tax="genres" class="tax-filter" data-term="<?php echo $tag->slug; ?>" ><?php echo ucfirst($tag->name); ?></a></li>
                        <?php 
                            $c++ ;
                            $c = ($c <= 5)? $c : 1 ;
                         }
            ...
            <?php
                ...
                if( $loop->have_posts() ){
                    while ($loop->have_posts()) {
                        $loop->the_post();
                        get_template_part('content', 'decouvertes');
                }//End while loop
            }//End if loop 
            wp_reset_postdata();
            ?>
